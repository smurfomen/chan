...
public:
    ...
    public:
        ...
        T receive() {
            ...
            cv->wait(lck, [this]{ return !tube->empty(); });
            T tmp = std::move(tube->front());
            tube->pop();
            return tmp;
        }
        ...
        opt_t receive(int64_t ms) {
            std::unique_lock<std::mutex> lck(*mtx);
            if(cv->wait_for(lck, std::chrono::milliseconds(ms), [this]{ return !tube->empty(); }))
            {
                T tmp = std::move(tube->front());
                tube->pop();
                return tmp;
